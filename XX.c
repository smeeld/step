...
glGenTextures(2,tex);
glGenBuffers(4,b);
loadtex(tex,"/home/g/s.jpg",1);
ang=0;
loadtex(tex,"/home/g/s.jpg",0);
int i;
SP=Sp(0.2,size);
ppar=Par(size1,-0.8);
glBindBuffer(GL_ARRAY_BUFFER,b[0]);
glBufferData(GL_ARRAY_BUFFER,sizeof(float)*size1*size1*3,(float*)ppar->vertex,GL_STREAM_DRAW);
glBindBuffer(GL_ARRAY_BUFFER,b[3]);
glBufferData(GL_ARRAY_BUFFER,sizeof(float)*size1*size1*8,(float*)ppar->texcoord,GL_STREAM_DRAW);
glBindBuffer(GL_ARRAY_BUFFER,b[1]);
glBufferData(GL_ARRAY_BUFFER,sizeof(float)*size*size*3,(float*)SP,GL_STREAM_DRAW);
glBindBuffer(GL_ARRAY_BUFFER,b[2]);
glBufferData(GL_ARRAY_BUFFER,sizeof(float)*3*size*size,(float*)SPn,GL_STREAM_DRAW);
